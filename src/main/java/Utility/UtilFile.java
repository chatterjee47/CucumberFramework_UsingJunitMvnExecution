...
public class UtilFile {
	...
	public void todaysdatepicker(String value){
		//This are the columns of the from date picker table
        List<WebElement> columns = driver.findElements(By.tagName("div"));
        for (WebElement cell: columns) {
            if (cell.getText().equals(value)) {
                cell.click();
                break;
	...
	public void ToolTipMouseOver(WebElement locator) {
		Actions builder = new Actions(driver);
	...
	public void ValidateDropdown(String value){
		List<WebElement> locator = driver.findElements(By.tagName("div"));
		int count = locator.size();
		for (int i = 0; i <count; i++) {
			String actualdata = locator.get(i).getText();
			if (actualdata.equals(value)) {
				locator.get(i).click();
				break;
		}
